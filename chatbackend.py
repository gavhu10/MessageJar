        ...
        match command:
            ...
            case 'delete': # delete room
                ...
                if not is_admin(author, room) or args != 'yes':
                    notify(f"User {author} is not an admin and cannot delete the room.", room)
                    return None
                else:
            ...
            case 'leave': # leave room
                if is_admin(author, room):
                    notify(f"Admin {author} cannot leave the room. Use \"/delete yes\" to delete the room.", room)
                    return None
                else:
